"""
App: Digitalizador de Formulários Teste 02
----------------------------------------------------------
Captura foto (câmera ou upload) de um formulário físico, usa o Gemini
para extrair as perguntas/respostas em JSON e salva automaticamente
como uma nova linha em uma planilha Excel local.
...
def obter_cliente_gemini() -> genai.Client:
    ...
    if not api_key:
        st.error(
            ...
            "Configure-a antes de rodar o app."
        )
        st.stop()
...
def extrair_dados_formulario(lista_imagens: list) -> dict:
    ...
    # Prepara o conteúdo unindo o prompt com todas as imagens carregadas
...
# Agora usamos uma lista para guardar todas as imagens enviadas
...
with aba_upload:
    # accept_multiple_files=True ativado
    arquivos = st.file_uploader("Selecione a(s) imagem(ns) do formulário", type=["png", "jpg", "jpeg", "webp"], accept_multiple_files=True)
    if arquivos:
        for arquivo in arquivos:
            imagens_para_processar.append((arquivo.getvalue(), arquivo.type or "image/jpeg"))

# Se houver pelo menos uma imagem carregada
if imagens_para_processar:
    st.info(f"{len(imagens_para_processar)} imagem(ns) pronta(s) para leitura.")
    
    # Exibe a pré-visualização de todas as imagens em colunas
